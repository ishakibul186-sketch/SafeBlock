...
// Rule IDs must be unique and >= 1
const ADULT_SITES_RULE_ID_START = 1;
const CUSTOM_URLS_RULE_ID_START = 10001;

// This function updates the blocking rules based on current settings.
async function updateBlockingRules() {
  try {
    const { settings } = await chrome.storage.local.get('settings');
    // If settings are not found, we can't do anything.
    if (!settings) {
      console.log('Safe Block: No settings found. Rules not updated.');
      return;
    }

    const rulesToAdd = [];
    
    // Add rules for adult sites if enabled
    if (settings.blockAdultSites) {
      MOCK_ADULT_SITES.forEach((domain, index) => {
        rulesToAdd.push({
          id: ADULT_SITES_RULE_ID_START + index,
          priority: 1,
          action: { type: 'block' },
          condition: {
            urlFilter: \`||\${domain}\`,
    ...
    if (settings.customBlockedUrls) {
      settings.customBlockedUrls.forEach((domain, index) => {
        if (domain && domain.trim()) {
          rulesToAdd.push({
            id: CUSTOM_URLS_RULE_ID_START + index,
            priority: 1,
            action: { type: 'block' },
            condition: {
    ...
    // Fetch existing rules to remove them before adding new ones
    ...
    console.error('Safe Block: Error updating blocking rules:', error);
  }
}

// We wrap the logic in an async function to ensure it completes during installation.
async function onInstall() {
  ...
  // After setting defaults, update the rules
  await updateBlockingRules();
}

// On install, run the setup function. The listener MUST wait for the async function to complete.
chrome.runtime.onInstalled.addListener(onInstall);

// When settings change, update the rules.
